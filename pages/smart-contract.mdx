# Develope, deploy and test a smart contract
...
```
$ aelf-command send pykr77ft9UUKJZLVq15wCH8PinBSjVRQ12sD1Ayq92mKFsJ1i ProposeNewContract -e http://127.0.0.1:8000
...
- Run the following command to get proposal_Id and proposedContractInputHash.

```
$ aelf-command event {transactionId} -e http://127.0.0.1:8000
```

- Run the following commands and type the Parliament contract address. Then select Approve option and type proposal_Id.
...
? Enter the required param <value>: {proposal_Id}
```

- Run the following commands to get new_proposal_Id.

```
$ aelf-command event {new_proposal_Id}
```

- Run the following commands to approve contracts.

```
$ aelf-command send pykr77ft9UUKJZLVq15wCH8PinBSjVRQ12sD1Ayq92mKFsJ1i ReleaseApprovedContract -e http://127.0.0.1:8000
