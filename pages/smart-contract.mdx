...
```
dotnet new install AElf.Contract --nuget-source http://10.0.1.239:8081/repository/nuget-group/index.json
```
...
Add the following code to the `src/Protobuf/contract/hello_world_contract.proto` file. This line of code will introduce a new method called `RandomCharacter`, which doesn't need any input and produces a string as output.
```
...
```
And also add this line of code into HelloWorldState.cs, this will encapsulate AEDPoS consensus contract reference state.
...
- Send aelf commands.
  - Open terminal, type aelf-command send, fill in the node with http://127.0.0.1:8000, address and password are same as before.
```
